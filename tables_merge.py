...
【打包兼容性说明】
- 已适配 PyInstaller 单文件模式（--onefile）
- 图标文件通过 --add-data "tables_merge.ico;." 打包
...
# ========== 【关键修改】资源文件路径适配（兼容 PyInstaller 打包环境） ==========
def resource_path(relative_path):
    ...
    """
    # 优先检查 PyInstaller 的 _MEIPASS（单文件模式临时解压目录）
...
try:
    ...
    # 【修改】打包后如果缺少tkinter，弹出更友好的错误（但tkinter是Python内置，一般不会缺）
...
class TableMergeApp:
    ...
    def _setup_icon(self):
        """【新增】统一设置窗口图标，兼容打包前后环境"""
        icon_files = ['tables_merge.ico', 'tables_merge.png', 'tables_merge.jpg']
        icon_set = False
            ...
            try:
                ...
                if os.path.exists(icon_path):
                    if icon_name.endswith('.ico'):
                        # Windows 优先使用 iconbitmap（支持 .ico）
                        self.root.iconbitmap(icon_path)
                        icon_set = True
                        break
                    else:
                        # 回退到 iconphoto（支持 .png/.jpg）
